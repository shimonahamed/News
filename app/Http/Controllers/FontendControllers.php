...
class FontendControllers extends Controller
{
    ...
    public function wcategory(){
        $data['categories'] = categoryModel::all();
        $data['categoryName'] = News::with('author:name,id')->where('category_id',)->paginate(1);
        $data['news'] = News::with('author:name,id')->where('category_id',)->paginate(3);
        $data['latestNews']=News::take(4)->skip(0)->orderBy('id','DESC')->get();
